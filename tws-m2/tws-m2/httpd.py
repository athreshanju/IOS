...
class HTTPServer:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM, socket.IPPROTO_TCP) as self.s:
            ...
            while True:
                ...
                with conn:
                    ...
                    req = conn.recv(1024)  #req from browser
                    f = str(req).split(" ") #splitting the received string 
                    uri = f[1]
                    # print(uri)
    ...
    def get_data(self, uri):
        ...
        '''
        if(uri == "/"):               #for checking if the uri contains this.

            data = "<h1>Webserver Under construction</h1>"  #we print this statement.
            return 200, "text/html" , len(data), data.encode()   #return the statement

        
        uri = os.getcwd() + "/www/"+uri       #for getting current working directory to know the path.

        if(os.path.isfile(uri)):                 #for checking if the uri is file or not.
        	url = mimetypes.MimeTypes().guess_type(uri)[0] #to find a file type in python.
        	data = open(uri,'rb').read()  #reading the file in bytes

        	return 200,url,len(data),data   #returns the data
        else:
        	data = "<h1>File Not Found</h1>"     #if the uri of file is not found
                                           

        	return 404, "text/html" , len(data),data.encode() #returns the data
